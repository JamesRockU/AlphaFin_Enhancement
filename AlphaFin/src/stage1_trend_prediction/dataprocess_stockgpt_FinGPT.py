...
def main(stockgpt_pred_path, mldl_pred_path, save_path):
    ...
    # --- 关键改动开始 ---
    # 将StockGPT结果拷贝到ML&DL表格中，并改名为 FinGPT
    df_all['FinGPT'] = df_stockgpt['StockGPT']  # <<< 新列名
    # --- 关键改动结束 ---
    ...
    # 将 FinGPT 的 0 值替换为 ML&DL 的预测结果
    for index, row in df_all.iterrows():
        fingpt_val = row['FinGPT']  # <<< 使用新列
        if fingpt_val == 0:
            col_mldl = [col for col in df_all.columns[3:] if col != 'FinGPT']  # <<< 排除新列
            df_mldl = row[col_mldl]
            pos_cnt = sum(df_mldl == 1)
            neg_cnt = sum(df_mldl == -1)
            df_all.at[index, 'FinGPT'] = 1 if pos_cnt > len(col_mldl) // 2 else -1  # <<< 写回新列
