...
# 读取 testdata_path 环境变量（如果设置了）
testdata_path = os.environ.get("testdata_path", None)
if testdata_path:
    print(f"[INFO] Loaded testdata path from env: {testdata_path}")
...
def getPredUpDownStrict(pred):
    ...
    if '因此' in pred or '综上所述' in pred:
        ...
        for b, kw in enumerate(keywords_up + keywords_down):
            if kw in pred:
                return 1 if b < len(keywords_up) else -1
    else:
        return 0
...
def main(stockgpt_pred_path, mldl_pred_path, save_path):
    ...
    df_stockgpt = pred_df
    df_all = pd.read_excel(mldl_pred_path, engine='openpyxl')  # stock_name, stock_code, date, ground_truth, RNN, ...
    ...
    # --- 关键改动开始 ---
    # 将StockGPT结果拷贝到ML&DL表格中，并改名为 StockGPT_RAG
    df_all['StockGPT_RAG'] = df_stockgpt['StockGPT']  # <<< 新列名
    # --- 关键改动结束 ---
    ...
    # 将 StockGPT_RAG 的 0 值替换为 ML&DL 的预测结果
    for index, row in df_all.iterrows():
        stockgpt_val = row['StockGPT_RAG']  # <<< 使用新列
        if stockgpt_val == 0:
            col_mldl = [col for col in df_all.columns[3:] if col != 'StockGPT_RAG']  # <<< 排除新列
            df_mldl = row[col_mldl]
            pos_cnt = sum(df_mldl == 1)
            neg_cnt = sum(df_mldl == -1)
            df_all.at[index, 'StockGPT_RAG'] = 1 if pos_cnt > len(col_mldl) // 2 else -1  # <<< 写回新列
